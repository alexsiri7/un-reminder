...
- `ci.yml` — the unit-test step runs `./gradlew test :app:verifyPaparazziRelease`, so a golden diff fails it. On a pull request that failure then runs `recordPaparazziRelease`, commits the new PNGs to the PR branch as `chore: update Paparazzi snapshots`, and fails once more so the image diff is reviewed before a re-run. That message deliberately carries no CI-skip marker: a squash merge concatenates the PR's commit subjects into `main`'s commit message, where a marker would suppress CI and Release for the merge (#351). The push does not re-trigger CI anyway, because it is made with the default `GITHUB_TOKEN`.
...
- **Manual regenerate:** the Cloud AI settings button enqueues a replace-mode `RefillWorker` run per active habit under its own unique work name (`regenerate-<id>`). The old pool keeps firing until the new batch lands; the swap (delete all unconsumed rows, insert the batch) is one transaction, and a failed or offline generation leaves the previous variants untouched (#402).
