...
class NotificationActionReceiverTest {
    ...
    fun `did it records the completion and refreshes the widget`() {
        coEvery { triggerRepository.recordOutcome(42L, TriggerStatus.COMPLETED) } returns true
    ...
    fun `dismiss records the dismissal and refreshes the widget`() {
        coEvery { triggerRepository.recordOutcome(42L, TriggerStatus.DISMISSED) } returns true

        receiver.onReceive(context, actionIntent(NotificationHelper.ACTION_DISMISSED))

        verify(timeout = 2_000) { pendingResult.finish() }
        coVerify(exactly = 1) { triggerRepository.recordOutcome(42L, TriggerStatus.DISMISSED) }
    ...
    fun `later records LATER, touches no dedication level, cancels and refreshes`() {
        coEvery { triggerRepository.recordOutcome(42L, TriggerStatus.LATER) } returns true

        receiver.onReceive(context, actionIntent(NotificationHelper.ACTION_LATER))

        verify(timeout = 2_000) { pendingResult.finish() }
        coVerify(exactly = 1) { triggerRepository.recordOutcome(42L, TriggerStatus.LATER) }
    ...
    fun `a swipe that lands after Later is ignored`() {
        coEvery { triggerRepository.recordOutcome(42L, TriggerStatus.DISMISSED) } returns false
    ...
    fun `two dismiss broadcasts for the same trigger record one dismissal`() {
        coEvery { triggerRepository.recordOutcome(42L, TriggerStatus.DISMISSED) } returnsMany listOf(true, false)
        receiver.onReceive(context, actionIntent(NotificationHelper.ACTION_DISMISSED))
        ...
        coVerify(exactly = 2) { triggerRepository.recordOutcome(42L, TriggerStatus.DISMISSED) }
        coVerify(exactly = 1) { dismissalTracker.onDismissed(42L) }
        verify(exactly = 1) { widgetRefresher.refresh() }
