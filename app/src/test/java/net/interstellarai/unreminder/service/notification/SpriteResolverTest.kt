...
class SpriteResolverTest {
    ...
    fun `known tag resolves to its own drawable`() {
        ...
        assertEquals(third.drawableRes, resolver.resolve(third.tag, rotationSeed = 0L, catalogue = catalogue))
    }
    ...
    fun `known tag wins over the rotation slot for the same trigger id`() {
        ...
        assertEquals(tagged.drawableRes, resolver.resolve(tagged.tag, rotationSeed = 0L, catalogue = catalogue))
    }
    ...
    fun `unknown tag falls back to rotation`() {
        assertEquals(
            ...
            resolver.resolve("sprite_from_a_retired_sheet", rotationSeed = 7L, catalogue = catalogue)
        )
    }
    ...
    fun `null tag falls back to rotation`() {
        assertEquals(
            catalogue[7L.mod(catalogue.size)].drawableRes,
            resolver.resolve(null, rotationSeed = 7L, catalogue = catalogue)
        )
    }
    ...
    fun `empty tag falls back to rotation`() {
        assertEquals(
            catalogue[7L.mod(catalogue.size)].drawableRes,
            resolver.resolve("", rotationSeed = 7L, catalogue = catalogue)
        )
    }

    @Test
    fun `fallback is stable for a given trigger id`() {
        val first = resolver.resolve(null, rotationSeed = 42L, catalogue = catalogue)
        val second = resolver.resolve(null, rotationSeed = 42L, catalogue = catalogue)
    ...
    fun `fallback wraps at the catalogue size`() {
        assertEquals(
            resolver.resolve(null, rotationSeed = 0L, catalogue = catalogue),
            resolver.resolve(null, rotationSeed = catalogue.size.toLong(), catalogue = catalogue)
        )
    }
    ...
    fun `negative trigger id maps into the catalogue`() {
        assertEquals(
            ...
            resolver.resolve(null, rotationSeed = -1L, catalogue = catalogue)
        )
    }
    ...
    fun `fallback derives the rotation size from a catalogue of a different size`() {
        ...
        assertEquals(threeSprites[1].drawableRes, resolver.resolve(null, rotationSeed = 4L, catalogue = threeSprites))
        assertEquals(threeSprites[0].drawableRes, resolver.resolve("gone", rotationSeed = 9L, catalogue = threeSprites))

        val singleSprite = catalogue.take(1)
        assertEquals(singleSprite[0].drawableRes, resolver.resolve(null, rotationSeed = 7L, catalogue = singleSprite))
    }
    ...
    fun `tag outside the supplied catalogue rotates even when the full catalogue knows it`() {
        ...
        assertEquals(
            ...
            resolver.resolve(absent.tag, rotationSeed = 1L, catalogue = threeSprites)
        )
    }
    ...
    fun `default catalogue is the sprite vocabulary`() {
        ...
        assertEquals(sprite.drawableRes, resolver.resolve(sprite.tag, rotationSeed = 0L))
    }
