...
**A home-screen widget.** Shows one currently-doable habit and a single action to mark it done; tapping the card opens the shown variant's view (the menu is one step back), and with no habit to show, the card opens the menu. Its content refreshes at least every 30 minutes, and also whenever the user's context changes in a way that alters what is doable. When nothing is doable it shows a resting state rather than a stale suggestion.
